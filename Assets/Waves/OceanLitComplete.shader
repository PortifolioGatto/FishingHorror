Shader "Custom/OceanLitComplete"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // Globals from BoatWakeTrail
            float  _WakeTrailCount;       // total count (single mode)
            float4 _WakeTrailPoints[32];  // xyz = world pos, w = startTime
            float4 _WakeTrailParams[32];  // x = width, y = foamIntensity, z = velocity, w = unused
            float  _WakeTrailFadeTime;
            float  _WakeTrailPortCount;   // number of port points (0..portCount-1)
            float  _WakeTrailStarCount;   // number of starboard points (16..16+starCount-1)
            float  _WakeTrailFoamColor;
            ...
            float WakeSegment(float3 worldPos, int idxA, int idxB)
            {
                ...
                // Project pixel onto line segment AB (XZ plane)
                ...
                // Time fade: smooth fade out over fadeTime
                float age = _Time.y - pointTime;
                if (age > _WakeTrailFadeTime) return 0.0;
                float lifeFraction = age / max(_WakeTrailFadeTime, 0.01);
                // Smooth ease-out: starts at 1, fades to 0
                float timeFade = 1.0 - lifeFraction;
                timeFade = timeFade * timeFade * (3.0 - 2.0 * timeFade); // smoothstep shape

                // Width grows slightly as trail ages (dissipation)
            ...
            {
                float foam = 0.0;
                int portCount = (int)_WakeTrailPortCount;
                int starCount = (int)_WakeTrailStarCount;

                if (portCount > 1 || starCount > 1)
                {
                    // Dual trail: port = 0..portCount-1, starboard = 16..16+starCount-1
                    for (int i = 0; i < portCount - 1; i++)
                        foam = max(foam, WakeSegment(worldPos, i, i + 1));

                    for (int j = 0; j < starCount - 1; j++)
                        foam = max(foam, WakeSegment(worldPos, 16 + j, 16 + j + 1));
                }
                else
                {
                    // Single trail
                    int count = (int)_WakeTrailCount;
